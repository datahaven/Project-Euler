				...
				int centrePillarHeight)
{
	...
	for(int i=n-1; i>=0; --i)
	{
		if ( itL->mBlocks[i] != 0 )
		{
			columns[colpos++] = itL->mBlocks[i];
			//cout << itL->mBlocks[i] << " ";
		}
	}
	
	int middle = colpos;
	columns[colpos++] = centrePillarHeight;
	...
	bool symmetryCheckNeeded = colpos-middle-1 == middle;
	return tryAllColPossibilities(columns, middle, symmetryCheckNeeded);
}
...
int countSculptures(int a, int b, int c)
{
		...
		itL != vLeftBlocks.end(); ++itL )
	{
			...
			itR != vRightBlocks.end(); ++itR )
		{
			...
			{
				// TODO - Still need to check we don't generate multiple
				// symmetrical copies when Left == Right
				// This is now in, although for speed I could put in a flag to
				// say when that code isn't needed
...
void sidesAndCentre()
{
	...
	cout << sculptureCount << " possible sculptures" << endl;
}

void MULTI_THREAD_sidesAndCentre()
{
	int sculptureCount = 0;
	...
	parallel_for( 1, n+1, [&](int b) {
		for(int a=0;a+b<=n;++a)
		{
			...
			int sculptureCountForShape = countSculptures(a, b, c);
			cout << "(" << a << ", " << b << ", " << c << ") = " << sculptureCountForShape << endl;
			localSum.local() += sculptureCountForShape;
		}
	});
...
bool checkBoard(char board[n][n], int colCount)
{
	int startX = -1;
	int startY = -1;
...
// TODO - Checksum of mirror == checksum of normal.
...
vector<SOLTYPE> boardList; // Global version
...
int tryAllColPossibilities(int cols[], int middle, bool symmetryCheckNeeded)
{
	...
	// vector<SOLTYPE> boardList; - local version
	while(running)
	{
